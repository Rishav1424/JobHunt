...
2. **Asynchronous Worker Layer**: Powered by BullMQ and Redis, this layer handles CPU-bound browser automation (scraping) and rate-limited LLM scoring sequentially.
...
│  - Gemini AI Engine (gemini-3.1-flash-lite / embedding)  │
└──────────────────────────────────────────────────────────┘
```
...
* **AI Engine**: **`@google/generative-ai`** SDK:
  ...
  * **`gemini-embedding-001`**: Generates 3072-dimensional vector representations for resume semantic searches.
...
* **[`backend/src/jobs/`](file:///c:/Users/Lenovo/Code/JobHunt/backend/src/jobs/)**: Job queues.
  * `queues.ts`: Declares BullMQ queues and worker logic (concurrency is set to `1` to prevent hitting free-tier Gemini API limits).
* **[`backend/src/services/scrapers/`](file:///c:/Users/Lenovo/Code/JobHunt/backend/src/services/scrapers/)**: Individual scrapers.
  ...
  * `index.ts`: The orchestrator. Coordinates scraping, applies keyword filtering, deduplicates listings, and persists unique records.
...
sequenceDiagram
    ...
    W->>G: Generate profile vector embedding (gemini-embedding-001) from LaTeX
    ...
    W->>DB: Save unique listings (status: NEW)
    W->>DB: Atomic update status (NEW -> SCORING)
    W->>Q: Bulk enqueue jobs into scoring queue
    
    loop Sequential Job Scoring
        Q->>W: Process scoring job
        W->>G: Generate job description embedding (3072-dim)
        ...
        W->>DB: Save fitScore & fitAnalysis, update status to SCORED
        W->>User: Socket.io real-time update (Dashboard updates view)
    end
    
    User->>DB: PATCH /status (User clicks Approve or Skip)
    DB->>Q: Write signal to Redis approved/skipped capped lists
    Note over User,DB: Future scores calibrate automatically using updated Redis signals!
```
